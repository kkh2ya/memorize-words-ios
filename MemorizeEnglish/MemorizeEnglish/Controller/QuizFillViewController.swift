...
class QuizFillViewController: UIViewController {
    ...
    @IBOutlet weak var memorizeViewExample: UILabel!
    ...
    @IBAction func nextButton(_ sender: UIBarButtonItem) {
        guard WordListSingleton.NOWFlag > 0 else {
            memorizeViewExample.isHidden = true
    ...
    func wordListPackShow() {
        var exampleArray = sharedWordList[WordListSingleton.randomStartFlag].example.components(separatedBy: " ")
        
        for exampleElement in exampleArray {
            if exampleElement == sharedWordList[WordListSingleton.randomStartFlag].exAnswer {
                exampleArray[exampleArray.index(of: exampleElement)!] = "＿＿"
            }
        }
            
        var exampleModified = ""
        for exampleElement in exampleArray {
            exampleModified += exampleElement + " "
        }
        
        memorizeViewExample.text = exampleModified
            
        randomAN = Int(arc4random_uniform(UInt32(4)))
        switch randomAN {
        case 0:
            answer1.text = sharedWordList[WordListSingleton.randomStartFlag].exAnswer
        case 1:
            answer2.text = sharedWordList[WordListSingleton.randomStartFlag].exAnswer
        case 2:
            answer3.text = sharedWordList[WordListSingleton.randomStartFlag].exAnswer
        case 3:
            answer4.text = sharedWordList[WordListSingleton.randomStartFlag].exAnswer
        default:
        ...
        for words in sharedWordList {
            answerList.append(words.exAnswer)
        }
    ...
    override func viewDidLoad() {
        ...
        memorizeViewExample.isHidden = false
